...
bool SimpleLRU::_key_cmp::operator()(const std::reference_wrapper<const std::string> &lhs, 
...
bool SimpleLRU::InsertHead (const std::string &key, const std::string &value) {
	...
	this->_lru_index[std::reference_wrapper<const std::string>(key)] = std::reference_wrapper<lru_node>(*cur);
	return true;
}
